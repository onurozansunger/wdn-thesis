#!/usr/bin/env bash
# Part 2 baseline: 5-seed Modena self-play with the Part-1 defender
# checkpoint as starting point.  Uses the 4-attacker MoE so we can mine
# attack vocabularies later (Phase 2).
#
# Defender ckpt: runs/temporal_moe/20260522_204723/best_model.pt (seed=1 of
# the 10-seed Part-1 baseline, F1 0.860, hidden_dim=64, router_hidden_dim=24).
set -u
...
for s in $(seq 1 5); do
    echo "[sp5] $(date +%H:%M:%S) start seed$s"
    PYTHONUNBUFFERED=1 python3 -u -m wdn.train_selfplay \
        ...
        --seed $s \
        > "$LOG/sp5_modena_seed$s.log" 2>&1
    echo "[sp5] $(date +%H:%M:%S) done seed$s"
done
echo "[sp5] ALL DONE"
