...
# Hourly multipliers for a typical municipal water demand cycle: a night
# trough, a sharp morning peak and a broader evening one. Scaling this
# toward 1.0 is what gives the hydraulics real temporal dynamics.
DIURNAL_PATTERN = [
    0.55, 0.45, 0.40, 0.40, 0.50, 0.75, 1.20, 1.50,
    1.45, 1.25, 1.10, 1.05, 1.10, 1.05, 1.00, 1.05,
    1.15, 1.35, 1.50, 1.40, 1.20, 1.00, 0.80, 0.65,
]


def _apply_diurnal_pattern(wn, amplitude: float, step_sec: int) -> None:
    """Attach a scaled diurnal demand pattern to every junction.

    Without this, a network whose .inp defines no pattern is solved at
    steady state: every snapshot in a scenario is identical, and a replayed
    reading is then indistinguishable from a genuine one except for the
    observation noise it lacks. ``amplitude`` scales the cycle about 1.0,
    so 0 reproduces the constant-demand behaviour exactly.
    """
    if amplitude <= 0:
        return
    mult = [1.0 + amplitude * (v - 1.0) for v in DIURNAL_PATTERN]
    wn.options.time.pattern_timestep = step_sec
    name = "wdn_diurnal"
    if name in wn.pattern_name_list:
        wn.remove_pattern(name)
    wn.add_pattern(name, mult)
    for node_name in wn.junction_name_list:
        for ts in wn.get_node(node_name).demand_timeseries_list:
            ts.pattern_name = name
...
def generate_dataset(cfg: GenerateConfig) -> tuple[WDNGraph, list[Snapshot]]:
    ...
    for s in range(cfg.num_scenarios):
        ...
        _apply_diurnal_pattern(
            wn, getattr(cfg, "demand_pattern_amplitude", 0.0),
            sim_hydraulic_sec)
