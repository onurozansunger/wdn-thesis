...
from wdn.models.temporal_multitask import (
...
class TemporalAttackRouter(nn.Module):
    ...
    def forward(
        ...
        temporal_stats = masked_temporal_statistics(p_obs, p_mask)
        halves_diff = temporal_stats["halves_diff"]
        log_std = temporal_stats["log_std"]
        n_changes = temporal_stats["n_changes"]
...
def temporal_moe_loss(
    ...
    # ``replay_class_id`` is the *compacted* id of the replay class, or
    # None when the dataset has no replay windows at all — in which case
    # every node keeps weight 1.
    replay_node_w = None
    if "num_nodes" in batch:
        N = batch["num_nodes"]
        attack_per_node = targets.repeat_interleave(N)            # (B*N,)
        replay_node_w = torch.ones_like(attack_per_node, dtype=pressure_pred.dtype)
        if replay_class_id is not None and replay_weight != 1.0:
            is_replay_window = (attack_per_node == replay_class_id).float()
            is_attacked = batch["pressure_anomaly"].float()
            replay_node_w = replay_node_w + (
                (replay_weight - 1.0) * is_replay_window * is_attacked)
