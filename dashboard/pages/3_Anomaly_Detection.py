...
with col_ctrl1:
    show_attacked = st.checkbox("Only show snapshots that contain attacks", value=True)
    ...
    snap_idx = st.selectbox(
        "Select snapshot",
        display_indices,
        format_func=lambda i: f"Snapshot {i+1}  ({'contains attacks' if i in attacked_indices else 'clean'})",
    )
with col_ctrl2:
    threshold = st.slider("Detection threshold", 0.0, 1.0, 0.5, 0.01)
with col_ctrl3:
    st.markdown(f"""
    <div style="font-size:0.8rem; opacity:0.55; margin-top:1.5rem;">
    Sensors with anomaly probability above the
    threshold are flagged as compromised.
...
for i in range(graph.num_nodes):
    ...
    hover_text.append(
        f"<b>Node {node_names[i]}</b><br>Type: {t}"
        f"<br>{STATUS[s]['label']}"
        f"<br>Anomaly probability: {p_anom_prob[i]:.3f}"
        f"<br>Ground truth: {'Attacked' if p_anom_true[i] > 0 else 'Clean'}"
    )
...
with col_net:
    fig = build_network_figure(
        graph, discrete_colors=node_colors, node_text=hover_text,
...
with col_side:
    ...
    fig = go.Figure(data=go.Heatmap(
        z=cm, x=["Predicted Clean", "Predicted Attack"],
        y=["Actually Clean", "Actually Attacked"],
    ...
    | | |
    |---|---|
    | Observed sensors | **{n_observed}** / {graph.num_nodes} |
    | Attacks in this snapshot | **{n_attacks}** |
...
# ── Per-node probability chart ──
st.markdown("##### Anomaly Probability per Node")

bar_colors = [RED if p_anom_true[i] > 0 else "rgba(77,166,255,0.6)" for i in range(graph.num_nodes)]
fig = go.Figure()
fig.add_trace(go.Bar(
    x=node_names, y=p_anom_prob,
    marker=dict(color=bar_colors, line=dict(width=0)),
    hovertemplate="Node %{x}<br>Probability: %{y:.3f}<extra></extra>",
))
fig.add_hline(
    y=threshold, line_dash="dot", line_color=ORANGE, line_width=2,
    annotation_text=f"threshold = {threshold:.2f}",
    annotation_position="top right",
    annotation_font=dict(size=11, color=ORANGE),
)
fig.add_trace(go.Bar(x=[None], y=[None], marker_color=RED, name="Actually attacked", showlegend=True))
fig.add_trace(go.Bar(x=[None], y=[None], marker_color="rgba(77,166,255,0.6)", name="Actually clean", showlegend=True))
fig.update_layout(**plotly_layout(
