...
from calibrate_received_joint import TARGET,REFERENCE,cal_bank
...
PURPOSE='ltown_received_joint_075_confirmation'
...
def freeze():
    ...
    if PURPOSE in record['generator_seeds']:
        ...
        record.setdefault('amendments',[]).append({'purpose':PURPOSE,'seeds':FRESH,'reserved_before_generation':True,
            'reason':'One qualified, frozen joint-feature 0.75 confirmation; no post-confirmation tuning'})
    ...
    frozen_write(CONFIRM/'design_frozen.json',{'candidate':decision['arm'],'fresh_sources':FRESH,'model_seeds':SEEDS,
        ...
        'success':'At least24/30 replay wins and positive mean replay gain in every fresh source; pooled mean preserved, other-family mean loss<=.005, clean FPR mean preserved. Source pooled loss<=.005, other-family loss<=.01, FPR increase<=.0001. Absolute FPR<=.005 for every run. Require mean replay>=.75 and every other-family mean>=.80 in addition to all protections; no tuning or second confirmation.',
    ...
    for source in FRESH:
        ...
        if not path.exists():
            ...
            for start in range(0,len(bank),50000):
                ...
                q=model.predict(np.column_stack((x,b)))
...
def aggregate():
    ...
    report={'means':means,'paired_deltas':delta,'source_deltas':source_delta,'replay_wins':wins,'checks':checks,
        ...
        'replay_target_0_75_achieved':all(checks.values()) and means['candidate']['replay']>=.75 and min(means['candidate'][f] for f in other)>=.80,
    ...
    # Resample source means, never treat repeated model seeds as independent datasets.
    rng=np.random.default_rng(7506)
    source_means=np.array([np.mean([value(p,'candidate','replay') for p in pairs if p['source']==s]) for s in FRESH])
    source_gains=np.array([source_delta[str(s)]['replay'] for s in FRESH])
    draws=rng.integers(0,len(FRESH),size=(10000,len(FRESH)))
    report['source_replay_means']={str(s):float(v) for s,v in zip(FRESH,source_means)}
    report['source_cluster_bootstrap_95_percentile']={
        'candidate_replay_mean':np.quantile(source_means[draws].mean(1),[.025,.975]).tolist(),
        'paired_replay_gain':np.quantile(source_gains[draws].mean(1),[.025,.975]).tolist(),
        'draws':10000,'seed':7506,'caveat':'Only six independent generator sources; percentile interval is descriptive.'}
