...
def train(config_path: str) -> Path:
    cfg = load_recon_train_config(config_path)
    set_seed(cfg.seed)

    run_info = make_run_dir(Path(cfg.run_dir))
    setup_logging(run_info.run_dir / "train_recon.log")
    logger.info("Run dir: %s", run_info.run_dir)

    shutil.copy(config_path, run_info.run_dir / "config_recon.yaml")
    with open(run_info.run_dir / "config_recon.json", "w", encoding="utf-8") as f:
    ...
    model = ReconGNN(node_in_dim, edge_in_dim, cfg.model.hidden_dim, cfg.model.num_layers, cfg.model.dropout)
    model.to(device)
    ...
    for epoch in range(cfg.optim.epochs):
        ...
        if val_metrics["loss"] < best_val:
            ...
            torch.save(model.state_dict(), run_info.run_dir / "models" / "recon.pt")
            with open(run_info.run_dir / "metrics" / "val_recon.json", "w", encoding="utf-8") as f:
